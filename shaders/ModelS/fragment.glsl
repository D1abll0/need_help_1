...
void main()
{
	...
	for(int i = 0; i < NUM_OF_LIGHTS; i++)
	{
		...
		//ambient = ambient * pow(attenuation, type[1]) * pow(intensity, type[2]);
		//diffuse = diffuse * pow(attenuation, type[1]) * pow(intensity, type[2]);
		//specular = specular * pow(attenuation, type[1]) * pow(intensity, type[2]);
