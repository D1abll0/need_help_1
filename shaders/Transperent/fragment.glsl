...
void main()
{
	...
	for(int i = 0; i < NUM_OF_LIGHTS; i++)
	{
		...
		vec4 ambT = texture(material.diffuse, TexCoords);
		if(ambT.a < 0.1) discard;
		vec4 diffT = texture(material.diffuse, TexCoords);
		if(diffT.a < 0.1) discard;
		vec4 specT = texture(material.diffuse, TexCoords);
		if(specT.a < 0.1) discard;
		
		vec3 ambient = light[i].ambient * vec3(ambT);
		vec3 diffuse = light[i].diffuse * diff * vec3(diffT);
		vec3 specular = light[i].specular * spec * vec3(specT);
